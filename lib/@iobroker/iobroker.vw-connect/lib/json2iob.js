  ...
  async parse(path, element, options) {
    try {
      ...
      if (typeof element === "string" || typeof element === "number") {
        ...
        await this.adapter.setState(path, element, true);
      ...
      for (const key of objectKeys) {
        ...
        if (Array.isArray(element[key])) {
          ...
          await this.adapter.setState(path + "." + pathKey, element[key], true);
        }
      }
    } catch (error) {
    ...
    try {
      ...
      for (let index in element) {
        ...
        if (
          ...
          await this.adapter.setState(path + "." + subKey, subValue, true);
          continue;
        }
