...
module.exports = class Json2iob {
  ...
  async extractArray(element, key, path, options) {
    try {
      ...
      let pureStringArray = true;
      for (let index in element) {
        const arrayElement = element[index];
        const indexNr = parseInt(index) + 1;
        if (indexNr < 10) {
          index = "0" + index;
        }
        ...
        await this.parse(newPath + "." + arrayPath, arrayElement, options);
      }
      if (pureStringArray == true) {
        this.setOtherStatesInChannelNull(path + "." + key, lastTimestamp);
      }
    } catch (error) {
  ...
  async setOtherStatesInChannelNull(channel, ts) {
    const states = await this.adapter.getStatesAsync(channel + ".*");
    const allIds = Object.keys(states);
    for (const keyName of allIds) {
      if (states[keyName] && states[keyName].ts < ts) {
        await this.adapter.setStateAsync(keyName, null, true);
      }
    }
  }
};